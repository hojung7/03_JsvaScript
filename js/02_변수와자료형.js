...
function scopeTest(){
  ...
  // num4 is not defined -> num4가 정의되지않음
}
...
/*  변수 선언 위치에 따른 범위 */
  ...
  function scopeTest2(){
    ...
    let fn2 = "함수 지역 범수2";    // 변수 (값 변경 O)
    const temp = 123; // 테스트를 위한 임시 값(함수 지역 변수)

     // const temp = 234;
     //[변수명 중복]
     // 블록 범위 변수 'temp'를 다시 선언할 수 없습니다.
  ...
  function typeCheck(){
    ...
    console.log("sight : ", age, typeof sight);
 ...
 const user = {
  ...
  console.log("user.pw : ", user.id, typeof user.pw);
  console.log("user.point : ", user.id, typeof user.point);
